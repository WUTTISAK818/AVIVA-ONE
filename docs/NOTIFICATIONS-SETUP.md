...
Vercel Cron ถูกตั้งไว้แล้วใน `vercel.json` (`0 1 * * *` = 08:00 เวลาไทย วันละ 1 ครั้ง)
> หมายเหตุ: Vercel **Hobby plan** อนุญาต cron วันละ 1 ครั้งเท่านั้น — ถ้าตั้งถี่กว่านี้ (เช่นวันละ 2 ครั้ง) **deployment จะ fail ทั้งโปรเจกต์** ถ้าอัปเป็น Pro แล้วค่อยเพิ่มเป็น `0 1,9 * * *` ได้
