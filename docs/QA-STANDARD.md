...
2. **OWASP Top 10 (ย่อสำหรับแอปเรา)** — ทุกการตรวจหมวด 4/9 ต้องเช็ค: Broken Access Control (RLS จริงใน pg_policies ไม่ใช่แค่ UI) · Injection (parameterized เสมอ, esc() ในเอกสารพิมพ์) · Secrets ไม่อยู่ใน repo/DB · ไฟล์แนบข้อมูลอ่อนไหวต้องเป็น signed URL
   **2.1 ตรวจสิทธิ์ระดับฐานข้อมูลให้ครบ 3 ชั้น (เพิ่ม 2026-09-22 — บทเรียนจริง):** RLS อย่างเดียวไม่พอ ต้องไล่ให้ครบทั้ง
   - **policy** — `pg_policies`: มี policy ไหนให้ `anon`/`{public}` ด้วย `USING(true)` บ้าง
   - **GRANT ของตาราง/view** — `information_schema.role_table_grants`: `anon` ไม่ควรมีสิทธิ์ใด ๆ นอกจากที่ตั้งใจ
   - **view ที่เป็น SECURITY DEFINER** — ข้าม RLS ของตารางต้นทาง ถ้า GRANT หลวมจะรั่ว/เขียนทะลุได้
   - **EXECUTE ของฟังก์ชัน** — `pg_proc.proacl`: ฟังก์ชัน trigger ไม่ควรเรียกผ่าน REST ได้
   *เหตุผล:* รอบตรวจ #39 พบว่า view 3 ตัวถูก GRANT ให้ `anon` ครบทุกสิทธิ์รวม INSERT/UPDATE/DELETE ทั้งที่ RLS ตั้งถูกหมด — เกณฑ์เดิมตรวจแค่ RLS จึงมองไม่เห็น
...
7. **กันงานค้างค้างข้ามเดือน (เพิ่ม 2026-09-22):** รายการที่สถานะ `✅ โค้ดเสร็จ` ต้องเลื่อนเป็น `✔️ ตรวจผ่าน` หรือ `🚫 ติดขัด` ภายใน **7 วัน** · ONE ต้องแนบ "รายการค้างเกิน 7 วัน" ท้ายรายงานทุกครั้งที่ปิดชุดงาน
   *เหตุผล:* `src/lib/money.ts` เขียนเสร็จ 3 ก.ค. แต่ถูกนำไปใช้จริงแค่ 2 ไฟล์ จนถึง 17 ก.ย. (2 เดือนครึ่ง) — ยอดเงินที่พิมพ์มีลูกน้ำจึงกลายเป็น 0 เงียบ ๆ ตลอดช่วงนั้น ทั้งที่รู้ปัญหาตั้งแต่ ก.ค.
