...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: delErr } = await db.from("line_links").delete().eq("user_email", email).is("linked_at", null);
    if (delErr) return NextResponse.json({ error: `failed to clean old codes: ${delErr.message}` }, { status: 500 });
    ...
    return NextResponse.json({ error: `link-code failed: ${err?.message ?? "unknown"}` }, { status: 500 });
  }
}
