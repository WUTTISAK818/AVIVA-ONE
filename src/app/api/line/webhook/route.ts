...
export async function POST(req: NextRequest) {
  ...
  for (const ev of body.events ?? []) {
    ...
    if (m) {
      const code = m[1];
      const { data: link } = await db.from("line_links").select("id").eq("link_code", code).eq("linked", false).maybeSingle();
      if (link) {
        await db.from("line_links").update({ line_user_id: userId, linked: true }).eq("id", link.id);
