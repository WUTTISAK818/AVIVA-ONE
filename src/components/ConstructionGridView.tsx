          ...
          {units.map((unit) => {
            ...
            return (
              ...
              >
                ...
                {/* SLA Days Late Indicator */}
                {unit.days_late && unit.days_late > 7 && (
                  <div className={`text-[8px] text-center mt-0.5 font-bold ${
                    unit.days_late >= 31 ? 'text-red-300' :
                    unit.days_late >= 15 ? 'text-orange-300' :
                    'text-yellow-300'
                  }`}>
                    {unit.days_late >= 31 ? '🚨' : unit.days_late >= 15 ? '🔴' : '🟡'} {unit.days_late}d
                  </div>
                )}
              </button>
            )
